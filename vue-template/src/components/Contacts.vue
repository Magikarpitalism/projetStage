...
export default {
  ...
  data() { // Component data
    return {
      dbPromise: openDB('contactDB', 1, { // Open a database and create a store named 'companies'
              upgrade(db) {
...
async fetchAllContacts(offset = 0, limit = 200) {
  ...
  if (response.data) {
    ...
    response.data.forEach(contact => store.put(contact));
...
async created() {
      const db = await this.dbPromise;
      this.allContacts = await db.getAll('contacts'); // Get all contacts from IndexedDB when the component is created
      if (this.allContacts.length === 0) { // If no contacts in IndexedDB, fetch them from API
